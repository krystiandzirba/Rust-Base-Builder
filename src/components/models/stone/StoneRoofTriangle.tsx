...
type GLTFResult = GLTF & {
  nodes: {Plane: THREE.Mesh}; //prettier-ignore
  materials: {["Material.002"]: THREE.MeshStandardMaterial}}; //prettier-ignore
...
export function Model(props: JSX.IntrinsicElements["group"]) {
  ...
  const { nodes, materials } = useGLTF("./models/stone/stone_roof_triangle_textured.glb") as GLTFResult;
      ...
      {roofs_active && !model_destroyed && (
        <group {...props} dispose={null}>
          <mesh
            ...
            {...defaultMeshMaterial(materials["Material.002"])}
          ...
          >
            {(!enable_model_textures || page_mode === "edit") && (
              <>
                ...
                />
                {meshEdgesVisibility}
                {meshAnnotationVisibility(["Stone roof (triangle)", "upgradeable", "non-downgradeable"])}
              </>
            )}
          </mesh>
        </group>
...
useGLTF.preload("./models/stone/stone_roof_triangle_textured.glb");
Model.displayName = "StoneRoofTriangle";
